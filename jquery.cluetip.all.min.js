...
A=e.find(u+"cluetip-title")}z=b("#cluetip-waitimage");z.length||(z=b("<div></div>").attr("id","cluetip-waitimage").css({position:"absolute"}));z.insertBefore(e).hide();var la=(parseInt(e.css("paddingLeft"),10)||0)+(parseInt(e.css("paddingRight"),10)||0);this.each(function(P){function q(){return false}var g=this,d=b(this),a=b.extend(true,{},t,b.metadata?d.metadata():b.meta?d.data():d.data("cluetip")||{}),E=false,I=false,ga=0,m=a[a.attribute]||d.attrProp(a.attribute)||d.attr(a.attribute),S=a.cluetipClass;
U=+a.cluezIndex;d.data("cluetip",{title:g.title,zIndex:U,selector:ba});if(!m&&!a.splitTitle&&!j)return true;if(a.local&&a.localPrefix)m=a.localPrefix+m;a.local&&a.hideLocal&&m&&b(m+":first").hide();var s=parseInt(a.topOffset,10),B=parseInt(a.leftOffset,10),C,ca,W=isNaN(parseInt(a.height,10))?"auto":/\D/g.test(a.height)?a.height:a.height+"px",X,v,w,J,Y,da=parseInt(a.width,10)||275,n=da+la+a.dropShadowSteps,F=this.offsetWidth,x,k,o,K,G,y=a.attribute!="title"?d.attrProp(a.titleAttribute)||"":"";if(a.splitTitle){G=
...
F=d.innerWidth();if(c.type==focus){o=x+F/2+B;e.css({left:k});J=v+s}else{o=c.pageX;J=c.pageY}if(g.tagName.toLowerCase()!="area"){X=b(document).scrollTop();K=b(window).width()}if(a.positionBy=="fixed"){k=F+x+B;e.css({left:k})}else{k=F>x&&x>n||x+F+n+B>K?x-n-B:F+x+B;if(g.tagName.toLowerCase()=="area"||a.positionBy=="mouse"||F+n>K)if(o+20+n>K){e.addClass("cluetip-"+S);k=o-n-B>=0?o-n-B-parseInt(e.css("marginLeft"),10)+parseInt(h.css("marginRight"),10):o-n/2}else k=o+B;f=k<0?c.pageY+s:c.pageY;if(k<0||a.positionBy==
"bottomTop")k=o+n/2>K?K/2-n/2:Math.max(o-n/2,0)}r.css({zIndex:d.data("cluetip").zIndex+1});e.css({left:k,zIndex:d.data("cluetip").zIndex});ca=b(window).height();if(j){if(typeof j=="function")j=j.call(g);h.html(j);L(f)}else if(G){c=G.length;h.html(c?G[0]:"");if(c>1)for(var i=1;i<c;i++)h.append('<div class="split-body">'+G[i]+"</div>");L(f)}else if(!a.local&&m.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)(?:\?.*)?$/i.test(m)){h.html('<img src="'+m+'" alt="'+y+'" />');L(f)}else{var l=a.ajaxSettings.beforeSend,
...
ha.call(g,D,H,e,h);h.html(E)}},complete:function(D,H){ia&&ia.call(g,D,H,e,h);var Z=h[0].getElementsByTagName("img");N=Z.length;for(var ea=0,ma=Z.length;ea<ma;ea++)Z[ea].complete&&N--;if(N&&!b.browser.opera)b(Z).bind("load error",function(){N--;if(N<1){z.hide();I&&L(f)}});else{z.hide();I&&L(f)}}});b.ajax(c)}else if(a.local){c=b(m+(/^#\S+$/.test(m)?"":":eq("+P+")")).clone(true).show();a.localIdSuffix&&c.attr("id",c[0].id+a.localIdSuffix);h.html(c);L(f)}},L=function(c){var f,i;f=y||a.showTitle&&"&nbsp;";
var l="";i="";e.addClass("cluetip-"+S);if(a.truncate){var M=h.text().slice(0,a.truncate)+"...";h.html(M)}f?A.show().html(f):A.hide();if(a.sticky){f=b('<div class="cluetip-close"><a href="#">'+a.closeText+"</a></div>");a.closePosition=="bottom"?f.appendTo(h):a.closePosition=="title"?f.prependTo(A):f.prependTo(h);f.bind("click.cluetip",function(){T();return false});a.mouseOutClose?e.bind("mouseleave.cluetip",function(){T()}):e.unbind("mouseleave.cluetip")}p.css({zIndex:d.data("cluetip").zIndex,overflow:W==
