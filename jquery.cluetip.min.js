...
var targetId=$(this).attr('id');return this.each(function(index){var link=this,$this=$(this);var opts=$.extend(true,{},$.fn.cluetip.defaults,options||{},$.metadata?$this.metadata():$.meta?$this.data():{});var cluetipContents=false;var cluezIndex=+opts.cluezIndex;$this.data('thisInfo',{title:link.title,zIndex:cluezIndex});var isActive=false,closeOnDelay=0;var cluetipId="cluetip-"+targetId;var cluetipIdWithSharp="#"+cluetipId;if(!$(cluetipIdWithSharp).length){$('body').append($('<div></div>').attr("id",cluetipId).append($('<div class="cluetip-outer"></div>').append($('<h3 class="cluetip-title"></h3>'),$('<div class="cluetip-inner"></div>')),$('<div class="cluetip-extra"></div>'),$('<div class="cluetip-arrows"></div>')));$cluetip=$(cluetipIdWithSharp).css({position:'absolute'});$cluetipOuter=$(cluetipIdWithSharp+' .cluetip-outer').css({position:'relative',zIndex:cluezIndex});$cluetipInner=$(cluetipIdWithSharp+' .cluetip-inner');$cluetipTitle=$(cluetipIdWithSharp+' .cluetip-title');$cluetipArrows=$(cluetipIdWithSharp+' .cluetip-arrows');$cluetipWait=$('<div class="cluetip-waitimage"></div>').css({position:'absolute'}).insertBefore($cluetip).hide();}
...
isActive=true;var cluetipIdWithSharp="#"+"cluetip-"+targetId;$cluetip=$(cluetipIdWithSharp).css({position:'absolute'});$cluetip.removeClass().css({width:tipInnerWidth});if(tipAttribute==$this.attr('href')){$this.css('cursor',opts.cursor);}
...
function doNothing(){};tipTitle?$cluetipTitle.show().html(tipTitle):(opts.showTitle)?$cluetipTitle.show().html('&nbsp;'):$cluetipTitle.hide();if(opts.sticky){var $closeLink=$('<div class="cluetip-close"><a href="#">'+opts.closeText+'</a></div>');(opts.closePosition=='bottom')?$closeLink.appendTo($cluetipInner):(opts.closePosition=='title')?$closeLink.prependTo($cluetipTitle):$closeLink.prependTo($cluetipInner);$closeLink.bind('click.cluetip',function(){cluetipClose();return false;});if(opts.mouseOutClose){$cluetip.bind('mouseleave.cluetip',function(){cluetipClose();});}else{$cluetip.unbind('mouseleave.cluetip');}}
...
$this.css('cursor','');if(opts.arrows){$cluetipArrows.css({top:''});}};$(document).bind('hideCluetip',function(e){cluetipClose();});if((/click|toggle/).test(opts.activation)){$this.bind('click.cluetip',function(event){if($cluetip.is(':hidden')||opts.sticky||!$this.is('.cluetip-clicked')){activate(event);$('.cluetip-clicked').removeClass('cluetip-clicked');$this.addClass('cluetip-clicked');}else{inactivate(event);}
