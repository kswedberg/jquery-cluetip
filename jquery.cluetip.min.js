...
A=e.find(u+"cluetip-title")}z=c("#cluetip-waitimage");z.length||(z=c("<div></div>").attr("id","cluetip-waitimage").css({position:"absolute"}));z.insertBefore(e).hide();var la=(parseInt(e.css("paddingLeft"),10)||0)+(parseInt(e.css("paddingRight"),10)||0);this.each(function(P){function q(){return false}var g=this,d=c(this),a=c.extend(true,{},t,c.metadata?d.metadata():c.meta?d.data():d.data("cluetip")||{}),H=false,I=false,ga=0,m=a[a.attribute]||d.attrProp(a.attribute)||d.attr(a.attribute),S=a.cluetipClass;
U=+a.cluezIndex;d.data("cluetip",{title:g.title,zIndex:U,selector:ba});if(!m&&!a.splitTitle&&!j)return true;if(a.local&&a.localPrefix)m=a.localPrefix+m;a.local&&a.hideLocal&&m&&c(m+":first").hide();var s=parseInt(a.topOffset,10),B=parseInt(a.leftOffset,10),C,ca,W=isNaN(parseInt(a.height,10))?"auto":/\D/g.test(a.height)?a.height:a.height+"px",X,v,w,J,Y,da=parseInt(a.width,10)||275,n=da+la+a.dropShadowSteps,E=this.offsetWidth,x,k,o,K,F,y=a.attribute!="title"?d.attrProp(a.titleAttribute)||"":"";if(a.splitTitle){F=
...
E=d.innerWidth();if(b.type==focus){o=x+E/2+B;e.css({left:k});J=v+s}else{o=b.pageX;J=b.pageY}if(g.tagName.toLowerCase()!="area"){X=c(document).scrollTop();K=c(window).width()}if(a.positionBy=="fixed"){k=E+x+B;e.css({left:k})}else{k=E>x&&x>n||x+E+n+B>K?x-n-B:E+x+B;if(g.tagName.toLowerCase()=="area"||a.positionBy=="mouse"||E+n>K)if(o+20+n>K){e.addClass("cluetip-"+S);k=o-n-B>=0?o-n-B-parseInt(e.css("marginLeft"),10)+parseInt(h.css("marginRight"),10):o-n/2}else k=o+B;f=k<0?b.pageY+s:b.pageY;if(k<0||a.positionBy==
"bottomTop")k=o+n/2>K?K/2-n/2:Math.max(o-n/2,0)}r.css({zIndex:d.data("cluetip").zIndex+1});e.css({left:k,zIndex:d.data("cluetip").zIndex});ca=c(window).height();if(j){if(typeof j=="function")j=j.call(g);h.html(j);L(f)}else if(F){b=F.length;h.html(b?F[0]:"");if(b>1)for(var i=1;i<b;i++)h.append('<div class="split-body">'+F[i]+"</div>");L(f)}else if(!a.local&&m.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)(?:\?.*)?$/i.test(m)){h.html('<img src="'+m+'" alt="'+y+'" />');L(f)}else{var l=a.ajaxSettings.beforeSend,
M=a.ajaxSettings.error,ha=a.ajaxSettings.success,ia=a.ajaxSettings.complete;b=c.extend(true,{},a.ajaxSettings,{cache:a.ajaxCache,url:m,beforeSend:function(D,G){l&&l.call(g,D,e,h,G);p.children().empty();a.waitImage&&z.css({top:J+20,left:o+20,zIndex:d.data("cluetip").zIndex-1}).show()},error:function(D,G){if(I)M?M.call(g,D,G,e,h):h.html("<i>sorry, the contents could not be loaded</i>")},success:function(D,G){H=a.ajaxProcess.call(g,D);if(c.isPlainObject(H)){y=H.title;H=H.content}if(I){ha&&ha.call(g,
D,G,e,h);h.html(H)}},complete:function(D,G){ia&&ia.call(g,D,G,e,h);var Z=h[0].getElementsByTagName("img");N=Z.length;for(var ea=0,ma=Z.length;ea<ma;ea++)Z[ea].complete&&N--;if(N&&!c.browser.opera)c(Z).bind("load error",function(){N--;if(N<1){z.hide();I&&L(f)}});else{z.hide();I&&L(f)}}});c.ajax(b)}else if(a.local){b=c(m+(/^#\S+$/.test(m)?"":":eq("+P+")")).clone(true).show();a.localIdSuffix&&b.attr("id",b[0].id+a.localIdSuffix);h.html(b);L(f)}},L=function(b){var f,i;f=y||a.showTitle&&"&nbsp;";var l=
"";i="";e.addClass("cluetip-"+S);if(a.truncate){var M=h.text().slice(0,a.truncate)+"...";h.html(M)}f?A.show().html(f):A.hide();if(a.sticky){f=c('<div class="cluetip-close"><a href="#">'+a.closeText+"</a></div>");a.closePosition=="bottom"?f.appendTo(h):a.closePosition=="title"?f.prependTo(A):f.prependTo(h);f.bind("click.cluetip",function(){T();return false});a.mouseOutClose?e.bind("mouseleave.cluetip",function(){T()}):e.unbind("mouseleave.cluetip")}p.css({zIndex:d.data("cluetip").zIndex,overflow:W==
