...
w=d.find(x+"cluetip-title")}B=c("#cluetip-waitimage");B.length||(B=c("<div></div>").attr("id","cluetip-waitimage").css({position:"absolute"}));B.insertBefore(d).hide();var la=(parseInt(d.css("paddingLeft"),10)||0)+(parseInt(d.css("paddingRight"),10)||0);this.each(function(p){function s(){return false}var i=this,e=c(this),a=c.extend(true,{},v,c.metadata?e.metadata():c.meta?e.data():{}),ea=false,H=false,fa=0,m=e.attrProp(a.attribute),Q=a.cluetipClass;S=+a.cluezIndex;e.data("cluetip",{title:i.title,
zIndex:S,selector:$});if(!m&&!a.splitTitle&&!j)return true;if(a.local&&a.localPrefix)m=a.localPrefix+m;a.local&&a.hideLocal&&m&&c(m+":first").hide();var u=parseInt(a.topOffset,10),C=parseInt(a.leftOffset,10),D,aa,U=isNaN(parseInt(a.height,10))?"auto":/\D/g.test(a.height)?a.height:a.height+"px",V,y,z,I,W,ba=parseInt(a.width,10)||275,n=ba+la+a.dropShadowSteps,F=this.offsetWidth,A,k,o,J,K,r=a.attribute!="title"?e.attrProp(a.titleAttribute):"";if(a.splitTitle){if(r==undefined)r="";K=r.split(a.splitTitle);
r=K.shift()}if(a.escapeTitle)r=r.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;");var Y=function(b){if(a.onActivate(e)===false)return false;H=true;d=c($).css({position:"absolute"});q=d.find(x+"cluetip-outer");g=d.find(x+"cluetip-inner");w=d.find(x+"cluetip-title");t=d.find(x+"cluetip-arrows");d.removeClass().css({width:ba});m==e.attr("href")&&e.css("cursor",a.cursor);a.hoverClass&&e.addClass(a.hoverClass);y=e.offset().top;A=e.offset().left;F=e.innerWidth();if(b.type==focus){o=A+F/2+
C;d.css({left:k});I=y+u}else{o=b.pageX;I=b.pageY}if(i.tagName.toLowerCase()!="area"){V=c(document).scrollTop();J=c(window).width()}if(a.positionBy=="fixed"){k=F+A+C;d.css({left:k})}else{k=F>A&&A>n||A+F+n+C>J?A-n-C:F+A+C;if(i.tagName.toLowerCase()=="area"||a.positionBy=="mouse"||F+n>J)if(o+20+n>J){d.addClass(" cluetip-"+Q);k=o-n-C>=0?o-n-C-parseInt(d.css("marginLeft"),10)+parseInt(g.css("marginRight"),10):o-n/2}else k=o+C;var f=k<0?b.pageY+u:b.pageY;d.css({left:k>0&&a.positionBy!="bottomTop"?k:o+n/
2>J?J/2-n/2:Math.max(o-n/2,0),zIndex:e.data("cluetip").zIndex});t.css({zIndex:e.data("cluetip").zIndex+1})}aa=c(window).height();if(j){if(typeof j=="function")j=j.call(i);g.html(j);L(f)}else if(K){b=K.length;g.html(b?K[0]:"");if(b>1)for(var h=1;h<b;h++)g.append('<div class="split-body">'+K[h]+"</div>");L(f)}else if(!a.local&&m.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)(?:\?.*)?$/i.test(m)){g.html('<img src="'+m+'" alt="'+r+'" />');L(f)}else{var l=a.ajaxSettings.beforeSend,ga=a.ajaxSettings.error,
